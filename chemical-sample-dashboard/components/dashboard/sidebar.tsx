...
const formatRoomTime = (room: ChatRoom, locale: string) => {
  ...
  if (!timestamp) return ""
  // Show time in the user's locale
  ...
  // Normalize UTC timestamps (append Z if missing)
...
export function DashboardSidebar({
  ...
  return (
    <aside className="flex h-full w-72 flex-col overflow-hidden bg-sidebar text-sidebar-foreground">
      <div className="border-b border-sidebar p-3 lg:p-4">
        <DropdownMenu>
        ...
        >
          ...
          {hasMore && (
            ...
            >
              ...
              {uiText.showMore} ({remainingCount})
            </button>
          )}
        </div>
      </div>
