...
def create_reagent(engine, payload: ReagentCreateRequest) -> Optional[ReagentItem]:
    row = reagents_repo.create_reagent(engine, {
        "reagent_name": payload.reagent_name, "formula": payload.formula, "purchase_date": payload.purchase_date,
        "open_date": None, "current_volume": payload.current_volume, "total_capacity": payload.total_capacity,
        "density": payload.density, "mass": payload.mass, "purity": payload.purity, "location": payload.location
    })
    return _row_to_reagent_item(row) if row else None
...
def list_disposals(engine, limit: int, cursor: Optional[int]) -> ReagentDisposalListResponse:
    ...
    # [중요] row.get("current_volume") 데이터를 포함하여 매핑합니다.
...
def list_storage_environment(engine) -> StorageEnvironmentResponse:
    ...
    items = [StorageEnvironmentItem(location=row.get("location") or "", temp=float(row.get("temp") or 0), humidity=float(row.get("humidity") or 0), status=row.get("status") or "normal") for row in rows]
